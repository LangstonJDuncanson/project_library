...
function getBooksBorrowedCount(books) {
  ...
  const booksWithBorrows = books.filter((book) => book.borrows.length !== undefined);
  let count = 0;
  for (const books in booksWithBorrows){
    const book = booksWithBorrows[books];
    const borrowedBooks = book.borrows;
    for(const borrowedBook in borrowedBooks){
      const currentBorrow = borrowedBooks[borrowedBook];
